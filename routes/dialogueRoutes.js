...
router.post('/start-dialogue', async (req, res) => {
    try {
        ...
        try {
            const generatedPath = await generateSpeech(firstResponse);
            parentAudioFilePath = `/audio/${path.basename(generatedPath)}`;
            console.log('家長第一句話語音檔案生成成功，URL:', parentAudioFilePath);
        } catch (error) {
...
//             console.log('語音檔案生成成功，URL:', audioFilePath);
//         } catch (error) {
...
6. 始終關注孩子的教育問題`;
        ...
        let audioFilePath = null;
        try {
            const generatedPath = await generateSpeech(aiResponse);
            audioFilePath = `/audio/${path.basename(generatedPath)}`;
            console.log('家長回應音頻生成成功，URL:', audioFilePath);
        } catch (error) {
